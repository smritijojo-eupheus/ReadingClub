...
export default function GradeTable({ handleData, grad, fetchData }) {
  ...
  const dataGrade = (grad) => {
    console.log("hiii", grad);
  };
  const fetch = async () => {
    console.log("hii", grad);
    const res = await instance({
      url: `grade/get/category/96efae1f-5cf4-4aa9-aa89-6ff4a58fc207`,
      method: "GET",
      headers: {
        Authorization: `${Cookies.get("token")}`,
        // accesskey: `auth74961a98ba76d4e4`,
      ...
      <div className="flex w-full">
        <TableContainer component={Paper} sx={{ minWidth: 500, height: "50%" }}>
          <Table aria-label="simple table">
            <TableHead className="!bg-primary !w-full">
              <TableRow>
